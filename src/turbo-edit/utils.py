...
def step_use_latents(
    ...
    if timestep_index < len(self._timesteps) - 1:
        x_t_minus_1_hat = x_t_minus_1_exact * (1. - self.object_mask) + x_t_minus_1_hat * self.object_mask
...
def get_ddpm_inversion_scheduler(
    ...
    if mask is None:
        scheduler.object_mask = torch.ones(x_ts[0].shape[-2:], device=x_ts[0].device, dtype=x_ts[0].dtype)
    else:
        scheduler.object_mask = mask.to(x_ts[0].device).to(x_ts[0].dtype)
    return scheduler
